...
if __name__ == "__main__":
    ...
    condition_1 = (df.Collective.isin([args.collective])) & (df['Report Fraction'].isin([args.fraction]))

    # Condition 2: picks tags
    condition_2_1 = ((df['Report Fraction'] == 0.25) & (df['Report Strategy'] == "tag") & df.Collective.isin([args.collective]))
    ...
    g = sns.lineplot(
        data=df[ condition_1 | condition_2 ],
        x=X_AXIS_TEXT,
        y="empr_harmfulness",
        hue="parsed_strategy",
        markers=True,
    ...
    tmp = df[ condition_1 | condition_2 ] #df[(df.Strategy == r"\textsc{Replace} (Ours)") & (df["Risk Control"] != "None") & (df["Risk Control"] != r"\textsc{Harm}")]
