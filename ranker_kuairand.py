...
if __name__ == "__main__":
    ...
    for run_id in range(args.runs):
        ...
        for k in tqdm([20], desc=f"Run {run_id}"):
            ...
            del size_of_recommendation, loss, ndcg_no_gamma, recall_no_gamma
            ...
            # --- record adversarial calibration stats for EVERY gamma in calibrate_gamma_precomputed grid ---
            if adversarial_users_set:
                calib_set_size = int(len(np.unique(user_ids_calibrations)))
                adv_users_present = [u for u in adversarial_users_set if np.any(user_ids_calibrations == u)]
                num_adv_users = int(len(adv_users_present))

                # Select predictions array consistent with args.score_type
                if args.score_type == "globalharm":
                    preds_for_calib = global_harmfulness_scores_calibration
                    scorer_for_calib = global_harm_scorer
                elif args.score_type == "harm":
                    preds_for_calib = harmfulness_rating_test_calibration
                    scorer_for_calib = harm_scorer
                elif args.score_type == "naive":
                    preds_for_calib = naive_scores_calibration
                    scorer_for_calib = naive_scorer
                else:
                    raise ValueError(f"Unknown score_type: {args.score_type}")

                for gamma_value, calib_score_adj in calibration_gammas_results:
                    avg_harm_adv_raw, num_adv_used = avg_harmfulness_for_users_raw(
                        user_ids=user_ids_calibrations,
                        items_ids=item_ids_calibration,
                        predictions=preds_for_calib,
                        feedbacks=ground_truth_calibration_harmfulness,
                        gamma=float(gamma_value),
                        k=k,
                        scoring_method=scorer_for_calib,
                        score=args.score_type,
                        method=args.method,
                        users_subset=adv_users_present,
                    )

                    adv_gamma_stats.append([
                        run_id,
                        args.epoch,
                        args.base_harm,
                        args.beta,
                        args.score_type,          # this identifies which scorer is used
                        args.method,
                        args.collective,
                        args.flag_strategy,
                        ...
                        args.topk_n,
                        float(gamma_value),
                        float(calib_score_adj),   # the adjusted calibration risk from calibrate_gamma_precomputed :contentReference[oaicite:5]{index=5}
                        num_adv_users,            # (1)
                        calib_set_size,           # (2)
                        float(avg_harm_adv_raw) if np.isfinite(avg_harm_adv_raw) else np.nan,  # (3) raw mean
                        num_poisoned,
                        num_total_items_adv
                    ])
            ...
            for alpha in tqdm(np.linspace(0.0, base_harm, num=25)[::-1], desc="Run alphas"):
                ...
                if args.score_type == "globalharm":
                    conformal_risk_score_list.append(("Global Harm", global_harm_scorer, gamma_for_this_alpha))
                elif args.score_type == "harm":
                    conformal_risk_score_list.append(("Harm", harm_scorer, gamma_for_this_alpha))
                elif args.score_type == "naive":
    ...
    if len(tag_frequency_results) > 0:
        tag_df = pd.DataFrame(
            ...
            columns=[
                ...
                "gamma", "alpha", "k",
                "Method", "Collective",
                "Report Strategy", "Report Fraction", "likes_softmax_temperature", "topk_n",
                "tag", "avg_frequency_in_topk",
            ],
        )
        tag_df.to_csv(
            f"tagfreq_{args.dataset}_"
            f"{args.method}_"
            f"{args.score_model}_"
            f"{args.score_type}_"
            f"{args.runs}_"
    ...
    if len(adv_gamma_stats) > 0:
        adv_gamma_df = pd.DataFrame(
            adv_gamma_stats,
            columns=[
                "run_id",
                "epoch",
                ...
                "avg_harm_adv_raw",
                "total_flagged_items",
                "total_items_seen_by_adv_users"
            ],
        )

        adv_gamma_df.to_csv(
            f"adv_alpha_stats_{args.dataset}_"
            f"{args.method}_"
            f"{args.score_model}_"
            f"{args.score_type}_"
            f"{args.runs}_"
