...
def process_user_only_model(user_ids_chunk, user_ids, item_ids, pred_ratings, true_ratings, feedbacks, k, return_recommendations: bool = False):
...
def process_user(user_ids_chunk, user_ids, item_ids, pred_ratings, true_ratings, feedbacks,
    ...
    for user_id in user_ids_chunk:
        ...
        if return_recommendations:
            # Store the top-k recommended item ids for tag-frequency analysis.
            # (May be < k when method='remove' filters many items.)
            recommended_topk = np.array(recommended_items_ids_for_user)[:k]
            results.append((
